...
function getConfig() {
    if (process.env.KUCOIN_API_KEY && process.env.KUCOIN_SECRET_KEY && process.env.KUCOIN_PASSPHRASE) {
        return {
            ...
            isSandbox: process.env.KUCOIN_IS_SANDBOX === 'true'
        };
    }
...
class KuCoinClient {
    constructor(config = {}) {
        ...
        this.isSandbox = config.isSandbox || process.env.KUCOIN_IS_SANDBOX === 'true';

        if (this.isSandbox) {
            this.baseUrl = 'api-sandbox.kucoin.com';
            this.futuresUrl = 'api-sandbox-futures.kucoin.com';
        } else {
    ...
    getMarginOrders(params = {}) {
        return this.request('/api/v3/margin/orders', 'GET', params);
    }
